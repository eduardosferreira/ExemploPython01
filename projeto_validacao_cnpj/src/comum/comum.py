...
def fnc_valide_cnpf(p_cc_dado: str) -> str:
    """
        Retorna a validação do cnpf,
...
def fnc_gere_cnpf_valido(p_qt_elementos: int = 1) -> list:
    """
        Retorna um cnpf valido
    """
    v_ob_lista = list()
    while len(v_ob_lista) < p_qt_elementos:
        v_cc_cnpj = ''.join([str(randint(0, 9)) for _ in range(0, 14)])
        if not fnc_valide_cnpf(v_cc_cnpj):
            v_ob_lista.append(v_cc_cnpj)
    return list(map(fnc_formata_cnpf, v_ob_lista))


def fnc_formata_cnpf(p_cc_dado: str) -> str:
    """
        Retorna a formatacao do cnpf,
    """
    try:
