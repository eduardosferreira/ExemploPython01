...
def fnc_encoding_arquivo(path_arquivo: str, mode_arquivo: str = ""):
    if str(mode_arquivo).upper().strip() in ("W", "A", "W+", "A+"):
        return 'utf-8'
    flag_erro: bool = False
    try:
        abertura_arquivo = open(file=path_arquivo, mode='r')
        linha_arquivo = abertura_arquivo.read()
    except Exception:
        try:
            abertura_arquivo = open(file=path_arquivo, mode='rb')
            linha_arquivo = abertura_arquivo.read()
        except Exception:
            flag_erro = True
    finally:
        try:
            abertura_arquivo.close()
        except Exception:
            pass
        if flag_erro:
            return 'utf-8'
    try:
        if type(linha_arquivo) == bytes:
            tipo_arquivo = linha_arquivo.decode('iso-8859-1')
        else:
            tipo_arquivo = linha_arquivo.encode('iso-8859-1')
        return 'iso-8859-1' if tipo_arquivo is not None else 'utf-8'
    except Exception:
        try:
            if type(linha_arquivo) == bytes:
                tipo_arquivo = linha_arquivo.decode('latin-1')
            else:
                tipo_arquivo = linha_arquivo.encode('latin-1')
            return 'latin-1' if tipo_arquivo is not None else 'utf-8'
        except Exception:
            return 'utf-8'
